...
"""
Source: https://github.com/Atcold/pytorch-PPUU/blob/master/models.py

* small modifications to fit 100x100 image inputs
"""
...
class encoder(nn.Module):
    """
    The encoder receives the input state sequence $s_{k:t}$ and an action $a$
    and encodes them into a hidden representation $h$. To do so, the network
    consists of three separate networks. The first one is a three-layered CNN
    that encodes the neighbourhood images. The second and third consist of three
    linear layers. All layers use the leakyRelu activation function and dropout
    as specified in a hyperparameter. The combined output gets calculated by
    summing the individual outputs together.
    """
    def __init__(self, opt, a_size, n_inputs, states=True, state_input_size=4, n_channels=3):
        ...
        if opt.layers == 3:
            ...
            # print('feature maps ', self.feature_maps)
            # print('ncond ', self.n_inputs)
            # print('hidden size ', opt.hidden_size)
            # print('nfeature ', opt.nfeature)
...
class u_network(nn.Module):
    def __init__(self, opt):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(self.opt.nfeature, self.opt.nfeature, 4, 2, 1),
            nn.Dropout2d(p=opt.dropout, inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose2d(self.opt.nfeature, self.opt.nfeature, 4, 2, 1)
        )
        ...
        self.hidden_size = self.opt.nfeature*3*3
    ...
    def forward(self, h):
        ...
        h2 = h2.view(h1.size())
        # print('h2 transformation size: ', h2.size())
        h3 = self.decoder(h2)
...
class decoder(nn.Module):
    def __init__(self, opt, n_out=1):
        ...
        if self.opt.layers == 3:
            ...
            self.f_decoder = nn.Sequential(
                ...
                nn.Dropout2d(p=opt.dropout, inplace=True),
                nn.LeakyReLU(0.2, inplace=True),
                nn.ConvTranspose2d(self.feature_maps[1], self.feature_maps[0], 4, 2, 0),
                nn.Dropout2d(p=opt.dropout, inplace=True),
                nn.LeakyReLU(0.2, inplace=True),
                nn.ConvTranspose2d(self.feature_maps[0], self.n_out*3, 4, 2, 0)
            )
    ...
    def forward(self, h):
        ...
        # print('pred_state size: ', pred_state.size())
        pred_image = self.f_decoder(h)
        # print('pred_image size: ', pred_image.size())
...
class FwdCNN(nn.Module):
    ...
    def forward(self, inputs, actions, target, sampling=None, z_dropout=None):
        ...
        for t in range(npred):
            ...
            h = h + a_emb
            # print('h size: ', h.size())
            u_hidden = self.u_network(h)
            # print('u_hidden size: ', u_hidden.size())
            h = h + self.u_network(h)
            pred_image, pred_state = self.decoder(h)
            pred_image = torch.sigmoid(pred_image + input_images[:, -1].unsqueeze(1))
            # since these are normalized, we are clamping to 6 standard deviations (if gaussian)
#            pred_state = torch.clamp(pred_state + input_states[:, -1], min=-6, max=6)
    ...
    def create_policy_net(self, opt):
        ...
        # added deterministic policy as in FwdCNN
...
class FwdCNN_VAE(nn.Module):
    ...
    def forward_single_step(self, input_images, input_states, action, z):
        ...
        pred_image, pred_state = self.decoder(h)
        pred_image = torch.sigmoid(pred_image + input_images[:, -1].unsqueeze(1))
        # pr ed_state = torch.clamp(pred_state + input_states[:, -1], min=-6, max=6)
        pred_state = pred_state + input_states[:, -1]
...
class DeterministicPolicy(nn.Module):
    def __init__(self, opt, context_dim=0, output_dim=None, n_channels=3):
        super().__init__()
        self.opt = opt
        self.n_channels = n_channels
    ...
    def forward(self, state_images, states, context=None, sample=True,
        ...
        # print('Deterministic policy encoder:')
        # print('state_images', state_images.size())
