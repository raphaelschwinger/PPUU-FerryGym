...
mfile = opt.model_file + '-syntetic-finetune' + '.model'
...
if os.path.isfile(mfile):
    ...
    if opt.warmstart == 1:
        prev_model = f'{opt.model_dir}/model=fwd-cnn-layers={opt.layers}-bsize={opt.batch_size}-ncond={opt.ncond}-npred={opt.npred}-lrt=0.0001-nfeature={opt.nfeature}-dropout={opt.dropout}-gclip=5.0'
        prev_model += '-warmstart=0-seed=1.step100000.model'
        
    else:
        prev_model = '/workspace/models/model=fwd-cnn-layers=3-bsize=16-ncond=10-npred=20-lrt=0.0001-nfeature=256-dropout=0.1-gclip=5.0-warmstart=0-seed=1-syntetic.step80000.model'
