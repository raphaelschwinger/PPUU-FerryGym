...
parser.add_argument('-mfile', type=str, default='/workspace/results/23-10-dnn-mix/model=fwd-cnn-layers=3-bsize=16-ncond=10-npred=20-lrt=0.0001-nfeature=256-dropout=0.1-gclip=5.0-warmstart=0-seed=1-syntetic.step60000.model')
...
opt.model_file = f'{opt.model_dir}/policy_networks/MPER_dnn_mix-only-state/'
...
print('[loading data]')
dataloader = DataLoader(opt, data_dir='/workspace/data/rev-moenk2/training/', dataframe='df_2022-04-01 12:00:01-to-target-0-4.pkl')
...
#    loss_c = F.mse_loss(pred_costs, target_costs, reduce=False).mean(2)
    if gamma < 1.0:
        ...
        loss_s *= gamma_mask
        loss_c *= gamma_mask
    return loss_i, loss_s.mean(), torch.zeros(1), loss_p.mean()

def compute_loss_test(targets, predictions, gamma=1.0, r=True):
    target_images, target_states = targets
    pred_images, pred_states, pred_costs, loss_p = predictions
...
#        loss_c = proximity_cost.mean() + opt.lambda_lane * lane_cost.mean()
    ...
    return 0, total_loss_s, total_loss_c, total_loss_policy, total_loss_p
...
def test(nbatches, npred):
    ...
    for i in range(nbatches):
        ...
        loss_i, loss_s, loss_c_, loss_p = compute_loss_test(targets, pred)
