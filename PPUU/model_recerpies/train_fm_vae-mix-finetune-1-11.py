...
parser.add_argument('-model_dir', type=str, default='models/1-11-fm-vae-synthetic-mix-finetune')
...
parser.add_argument('-warmstart', type=int, default=1, help='initialize with pretrained model')
...
parser.add_argument('-tensorboard_dir', type=str, default='models',
                    help='path to the directory where to save tensorboard log. If passed empty path' \
                         ' no logs are saved.')
parser.add_argument('-synthetic', type=float, default=0.2, help='fraction of synthetic training data')
...
# parameters specific to the AIS dataset
opt.n_inputs = 4
opt.n_actions = 2
opt.height = 100
opt.width = 100
...
if os.path.isfile(mfile):
    ...
    if opt.warmstart == 1:
        prev_model = '/workspace/results/1-11-vae-mix/model=fwd-cnn-vae-fp-layers=3-bsize=8-ncond=10-npred=20-lrt=0.0001-nfeature=256-dropout=0.1-nz=32-beta=1e-06-zdropout=0.5-gclip=5.0-warmstart=1-seed=1.step100000.model' 
...
def train(nbatches, npred):
    ...
    for i in range(nbatches):
        optimizer.zero_grad()
        # sample batch with probability opt.synthetic from synthetic dataset
        if random.random() < opt.synthetic:
            inputs, actions, targets = dataloader.get_syntetic_batch_fm()
        else:
            inputs, actions, targets = dataloader.get_batch_fm('train', npred, onlyTurning=True)
